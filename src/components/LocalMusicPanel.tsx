...
export function LocalMusicPanel({ compact }: LocalMusicPanelProps) {
  const { localSongs, localLoading, importFiles, removeLocalSong } = useSongCatalog()
      ...
      <div className={`rounded-xl bg-surface-highlight/50 ${compact ? 'p-3' : 'p-4'} space-y-3`}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-white/50 mt-0.5">支持 MP3、FLAC、WAV 等，文件保存在浏览器本地</p>
          </div>
          <button
            type="button"
            disabled={importing}
            onClick={() => inputRef.current?.click()}
            className="shrink-0 px-4 py-2 rounded-full bg-white text-black text-sm font-medium hover:bg-white/90 active:scale-95 transition-all disabled:opacity-50"
          >
            {importing ? '导入中…' : '选择文件'}
          </button>
          <input
        ...
        {localLoading ? (
          ...
          <p className="text-sm text-white/40">暂无本地歌曲，点击上方按钮导入</p>
        ) : (
