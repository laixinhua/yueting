...
import { isCoverCachedInMemory } from '../utils/coverImageCache'
...
export function PlaylistCard({ playlist, onClick, variant = 'square', scrollable = false }: PlaylistCardProps) {
  ...
  const [coverReady, setCoverReady] = useState(() => isCoverCachedInMemory(playlist.coverUrl))
  useEffect(() => {
    setImgFailed(false)
    setCoverReady(isCoverCachedInMemory(playlist.coverUrl))
  }, [playlist.coverUrl, playlist.id])

  const showCover = Boolean(playlist.coverUrl) && !imgFailed
  const showGradientBg = !showCover || !coverReady
    ...
    return (
      ...
      >
        <div
          ...
          style={showGradientBg ? gradientStyle(playlist.gradient) : undefined}
        >
          {showCover ? (
            <CoverImage
