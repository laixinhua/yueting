...
export function AlbumCover({
  ...
  const [coverReady, setCoverReady] = useState(() => isCoverCachedInMemory(imageUrl))
  useEffect(() => {
    setImgFailed(false)
    setCoverReady(isCoverCachedInMemory(imageUrl))
  }, [imageUrl])

  const showImage = Boolean(imageUrl) && !imgFailed
  const showGradientBg = !showImage || !coverReady

  return (
    <div
      className={`relative overflow-hidden shrink-0 ${sizeMap[size]} ${roundedMap[rounded]} ${className} shadow-lg shadow-black/30`}
      style={showGradientBg ? gradientStyle(gradient) : undefined}
    >
      {showImage ? (
        <CoverImage
          src={imageUrl}
