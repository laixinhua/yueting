...
import { PLAYABLE_SEARCH_LIMIT, filterPlayableNeteaseSongs } from '../utils/neteasePlayable'
  ...
  useEffect(() => {
    ...
    searchTracks(q, 30)
      .then(async (tracks) => {
        ...
        setSongs(await filterPlayableNeteaseSongs(songs, { limit: PLAYABLE_SEARCH_LIMIT }))
      })
      .catch((err: unknown) => {
        if (cancelled) return
        setSongs([])
