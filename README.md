...
|------|------|
| 关键词搜索 | 提交后打开全屏结果页 |
| 在线搜索 | **EBNR（网易云全曲）+ iTunes（30 秒试听）** 双源并行，结果经可播放性过滤 |
| 热搜词 | 点击芯片直接搜索；支持 **换一换** 轮换词池 |
...
| 来源 | 用途 |
|------|------|
| **EBNR（网易云）** | 在线搜索 / 播放地址 / 歌单 / 专辑 / 热榜源（`https://ebnr.xiyang6666.top`） |
| **iTunes** | 在线搜索第二源，免 Key 官方 API（`https://itunes.apple.com/search`），返回 30 秒试听直链 |
...
- 搜索在线源为 **EBNR（网易云全曲）+ iTunes（30 秒试听）** 双源并行；CC 曲库为独立专区，不并入搜索结果
- 搜索页无流派分类筛选 UI
